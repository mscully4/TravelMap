from GooglePhotos import GooglePhotosAPI
...
class Manager(object):
    ...
    def get_archive_sequence(self):
        ...
        operation_parameters = {'Bucket': bucket_name, 'Prefix': 'archive/'}
    ...
    def delete_city(self):
        ...
        selected_city = try_cast(inp, int) - 1
    ...
    def edit_city(self, override_city=None):
        ...
        for k, v in self.data['destinations'][selected_city].items():
            if isinstance(v, (str, int, float)):
                ...
                inp = rlinput('{}: '.format(k), str(v) if isinstance(v, (str, int, float)) else "")
    ...
    def add_place(self, override_city=None):
        ...
        selection = try_cast(selection, int) - 1
        assert selection != None
        if selection == -1:
            cls()
    ...
    def edit_place(self, override_city=None, override_place=None):
        ...
        if override_place == None:
            ...
            selected_place = try_cast(inp, int) - 1
    ...
    def add_album(self, override_city=None, override_place=None, allow_add_another=True):
        ...
        if override_place == None:
            ...
            selected_place = try_cast(input('Selection: '), int) - 1
            assert selected_place != None
...
if __name__ == '__main__':
    ...
    gp = GooglePhotosAPI(GOOGLE_PHOTOS_CLIENT_SECRET_FILE, GOOGLE_PHOTOS_SCOPES)

    file_name, bucket_name = 'data.json', 'mscully-travel-map'
    m = Manager(bucket_name, file_name, gp)
    m.menu()